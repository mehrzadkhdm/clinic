...
StaffCancellationWidget::StaffCancellationWidget(const char *conninfo, UserSession *session, WContainerWidget *parent) :
...
void StaffCancellationWidget::studyList()
{
    ...
    SleepStudies studies = dbsession.find<SleepStudy>().where("appt_status = ?").bind("Cancelled").orderBy("std_date desc");
    ...
    for (SleepStudies::const_iterator i = studies.begin(); i != studies.end(); ++i, ++row)
    {
        ...
        new Wt::WText(Wt::WString::fromUTF8("{1}").arg(row),
            ...
            btnSelectSleepStudy->clicked().connect(boost::bind(&StaffCancellationWidget::showStudy, this, (*i)->studyId_));
    ...
    dbo::Transaction notrans(dbsession);

    typedef dbo::collection< dbo::ptr<SleepStudy> > SleepStudies;
    SleepStudies nostudies = dbsession.find<SleepStudy>().where("appt_status = ?").bind("No Show").orderBy("std_date desc");
    std::cerr << nostudies.size() << std::endl;

    Wt::WTable *notable = new Wt::WTable();
    notable->setHeaderCount(1);

    notable->elementAt(0, 0)->addWidget(new Wt::WText("#"));
    notable->elementAt(0, 1)->addWidget(new Wt::WText("Patient First Name"));
    notable->elementAt(0, 2)->addWidget(new Wt::WText("Patient Last Name"));
    notable->elementAt(0, 3)->addWidget(new Wt::WText("Patient Date of Birth"));
    notable->elementAt(0, 4)->addWidget(new Wt::WText("Study Date"));
    notable->elementAt(0, 5)->addWidget(new Wt::WText("Study Location"));
    notable->elementAt(0, 6)->addWidget(new Wt::WText("Appointment Status"));
    notable->elementAt(0, 7)->addWidget(new Wt::WText("Details"));


    int nrow = 1;
    for (SleepStudies::const_iterator n = nostudies.begin(); n != nostudies.end(); ++n, ++nrow)
    {

        new Wt::WText(Wt::WString::fromUTF8("{1}").arg(nrow),
            notable->elementAt(nrow, 0)),
            new Wt::WText((*n)->ptFirstName_, notable->elementAt(nrow, 1)),
            new Wt::WText((*n)->ptLastName_, notable->elementAt(nrow, 2)),
            new Wt::WText((*n)->ptDob_, notable->elementAt(nrow, 3)),
            new Wt::WText((*n)->studyDate_.toString("MM/dd/yyyy"), notable->elementAt(nrow, 4)),
            new Wt::WText((*n)->studyLocation_, notable->elementAt(nrow, 5)),
            new Wt::WText((*n)->appointmentStatus_, notable->elementAt(nrow, 6)),
            btnSelectSleepStudyNo = new Wt::WPushButton("View Study", notable->elementAt(nrow, 7)),
            //click 'PDF' button on any notable nrow, connect downloadReferral with that referral uuid
            btnSelectSleepStudyNo->clicked().connect(boost::bind(&StaffCancellationWidget::showStudy, this, (*n)->studyId_));
    ...
    notable->addStyleClass("table form-inline");
    notable->addStyleClass("table table-striped");
    notable->addStyleClass("table table-hover");

    container->addWidget(new Wt::WText("<h4>Cancellations</h4>"));
    container->addWidget(new Wt::WBreak());
    container->addWidget(table);
    container->addWidget(new Wt::WBreak());
    container->addWidget(new Wt::WText("<h4>No Shows</h4>"));
    container->addWidget(notable);

    addWidget(container);

}

WString StaffCancellationWidget::setCommentId()
{
    return uuid();
}
WString StaffCancellationWidget::setStdId()
{
    return uuid();
}
void StaffCancellationWidget::showStudy(Wt::WString& std)
{
    ...
    */
    ...
    Wt::WPushButton *completestudy = new Wt::WPushButton("Study Complete");
    stdright->addWidget(new Wt::WBreak());
    stdright->addWidget(new Wt::WBreak());
