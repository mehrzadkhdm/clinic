...
#include "LocalityReferralCountEntry.h"
#include "Referral.h"
#include "Practice.h"
#include "Locality.h"
...
LocalityReferralCountWidget::LocalityReferralCountWidget(const char *conninfo, UserSession *session, WContainerWidget *parent) :
...
void LocalityReferralCountWidget::showReferralCount()
{
    Wt::WContainerWidget *container = new Wt::WContainerWidget();

    dbo::Transaction transaction(dbsession);

    Wt::log("notice") << "localities is being queried for with start and end date";

    //define logentries vector here
    std::vector<LocalityReferralCountEntry> entries;
    ...
    {
        LocalityReferralCountEntry *ent = new LocalityReferralCountEntry();
        ...
        Practices practices = dbsession.find<Practice>().where("local_id= ?").bind((*i)->localityId_);
        ...
        for (Practices::const_iterator p = practices.begin(); p != practices.end(); ++p)
        {
            ...
            PracReferrals pracreferrals = dbsession.find<Referral>().where("prac_id= ?").bind((*p)->practiceId_);
            std::cerr << pracreferrals.size() << std::endl;

            for (PracReferrals::const_iterator r = pracreferrals.begin(); r != pracreferrals.end(); ++r)
            {

                recvDate = (*r)->refReceivedAt_;
    ...
    table->elementAt(0, 0)->addWidget(new Wt::WText("City Name"));
    table->elementAt(0, 1)->addWidget(new Wt::WText("Referrals This Month"));
    ...
    int row = 1;
    for (std::vector<LocalityReferralCountEntry>::iterator en = entries.begin(); en != entries.end(); ++en, ++row)
    {
            ...
            new Wt::WText(en->localityName, table->elementAt(row, 0)),
