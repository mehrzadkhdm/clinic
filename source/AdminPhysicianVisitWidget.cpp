...
AdminPhysicianVisitWidget::AdminPhysicianVisitWidget(const char *conninfo, UserSession *session, WContainerWidget *parent) :
...
void AdminPhysicianVisitWidget::visitList()
{
	...
	refreshTimer->timeout().connect(std::bind([=] () {
		...
		AdminPhysicianVisitWidget::search("");
	}));
	...
	edit->textInput().connect(std::bind([=] () {
	   ...
	   AdminPhysicianVisitWidget::search(edit->text().toUTF8());
	   
	   if (edit->text() == "")
	   {
			AdminPhysicianVisitWidget::startRefreshTimer();
	   } else {
	   		AdminPhysicianVisitWidget::stopRefreshTimer();
	   }
	...
	AdminPhysicianVisitWidget::search(Wt::WString(""));
	AdminPhysicianVisitWidget::startRefreshTimer();

}

void AdminPhysicianVisitWidget::preSearch(Wt::WString searchString)
{
	AdminPhysicianVisitWidget::search(Wt::WString(searchString));
	AdminPhysicianVisitWidget::startRefreshTimer();
}
void AdminPhysicianVisitWidget::stopRefreshTimer()
{
...
void AdminPhysicianVisitWidget::startRefreshTimer()
{
...
void AdminPhysicianVisitWidget::search(Wt::WString searchString)
{
	int limit = 100;
	int offset = 0;
    ...
    prContainer->done().connect(std::bind([=] () {
		...
		AdminPhysicianVisitWidget::preSearch(searchString);
	}));
}

WString AdminPhysicianVisitWidget::setIssueId()
{
	return uuid();
}

void AdminPhysicianVisitWidget::newPhysicianIssueDialog(Wt::WString prac, Wt::WString vid)
{
