...
const char *techconninfo = "hostaddr = 127.0.0.1 port = 5432 user = clinicore dbname = clinicore password = Falcon1337";

TechApplication::TechApplication(const Wt::WEnvironment& env)
    : Wt::WApplication(env),
    session_(techconninfo)
{
    session_.login().changed().connect(this, &TechApplication::authEvent);
    ...
    WApplication::instance()->internalPathChanged()
        .connect(this, &TechApplication::handleInternalPath);

}

void TechApplication::authEvent() {
...
void TechApplication::handleInternalPath(const std::string &internalPath)
{
    if (session_.login().loggedIn()) {
...
void TechApplication::showTonight()
{
    mainStack_->clear();
    Wt::WString dny = "Downey";
    tonight_ = new TechSleepStudyWidget(techconninfo, dny, &session_, mainStack_);
    tonight_->studyList();
...
void TechApplication::showBackup()
{
    mainStack_->clear();
    backup_ = new TechBackupListWidget(techconninfo, &session_, mainStack_);
    backup_->backupList();
...
void TechApplication::showShiftNote()
{
    mainStack_->clear();
    shiftnote_ = new TechShiftNoteWidget(techconninfo, &session_, mainStack_);
    shiftnote_->noteList();
...
void TechApplication::showPatient()
{
    mainStack_->clear();
    patient_ = new StaffPatientWidget(techconninfo, &session_, mainStack_);
...
{
    return new TechApplication(env);
}
