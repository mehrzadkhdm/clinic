...
StaffInboundWidget::StaffInboundWidget(const char *conninfo, UserSession *session, WContainerWidget *parent) :
...
void StaffInboundWidget::searchList()
{
    ...
    edit->textInput().connect(std::bind([=] () {
        ...
        StaffInboundWidget::search(upper);
        
        if (edit->text() == "")
        {
            StaffInboundWidget::startRefreshTimer();
        } else {
               StaffInboundWidget::stopRefreshTimer();
        }
    ...
    StaffInboundWidget::search(Wt::WString(""));
    StaffInboundWidget::startRefreshTimer();

}

void StaffInboundWidget::preSearch(Wt::WString searchString)
{
    StaffInboundWidget::search(Wt::WString(searchString));
    StaffInboundWidget::startRefreshTimer();
}

void StaffInboundWidget::stopRefreshTimer()
{
...
void StaffInboundWidget::startRefreshTimer()
{
...
void StaffInboundWidget::search(Wt::WString searchString)
{
    int limit = 100;
    int offset = 0;
    ...
    inboundContainer->done().connect(std::bind([=] () {
        ...
        StaffInboundWidget::preSearch(searchString);
    }));
}
