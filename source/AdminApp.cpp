...
const char *adminconninfo = "hostaddr = 127.0.0.1 port = 5432 user = clinicore dbname = clinicore password = Falcon1337";

AdminApplication::AdminApplication(const Wt::WEnvironment& env)
    : Wt::WApplication(env),
    session_(adminconninfo)
{
    ...
    useStyleSheet("css/adminstyle.css");
    useStyleSheet("css/composer.css");

    loginStack_ = new WStackedWidget(root());
    loginStack_->setStyleClass("loginStack");
    template_ = new AdminAppTemplate(adminconninfo, &session_, loginStack_);
    login_ = new AdminLoginTemplate(&session_, loginStack_);
    authEvent();
