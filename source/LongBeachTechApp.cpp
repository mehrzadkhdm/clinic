...
const char *lbconninfo = "hostaddr = 127.0.0.1 port = 5432 user = clinicore dbname = clinicore password = Falcon1337";

LongBeachTechApplication::LongBeachTechApplication(const Wt::WEnvironment& env)
    : Wt::WApplication(env),
    session_(lbconninfo)
{
    session_.login().changed().connect(this, &LongBeachTechApplication::authEvent);
    ...
    WApplication::instance()->internalPathChanged()
        .connect(this, &LongBeachTechApplication::handleInternalPath);

}

void LongBeachTechApplication::authEvent() {
...
void LongBeachTechApplication::handleInternalPath(const std::string &internalPath)
{
    if (session_.login().loggedIn()) {
...
void LongBeachTechApplication::showHome()
{
    mainStack_->clear();
...
void LongBeachTechApplication::showTonight()
{
    
    mainStack_->clear();
    Wt::WString lb = "Long Beach";
    tonight_ = new TechSleepStudyWidget(lbconninfo, lb, &session_, mainStack_);
    tonight_->studyList();
...
void LongBeachTechApplication::showBackup()
{
    mainStack_->clear();
    backup_ = new TechBackupListWidget(lbconninfo, &session_, mainStack_);
    backup_->backupList();
...
void LongBeachTechApplication::showShiftNote()
{
    mainStack_->clear();
    shiftnote_ = new TechShiftNoteWidget(lbconninfo, &session_, mainStack_);
    shiftnote_->noteList();
...
{
    return new LongBeachTechApplication(env);
}
