...
StaffBackupListWidget::StaffBackupListWidget(const char *conninfo, UserSession *session, WContainerWidget *parent) :
...
void StaffBackupListWidget::backupList()
{
    ...
    table->elementAt(0, 5)->addWidget(new Wt::WText("More"));
    ...
    for (Backups::const_iterator i = backups.begin(); i != backups.end(); ++i, ++row)
    {
        ...
        new Wt::WText(Wt::WString::fromUTF8("{1}").arg(row),
            ...
            btnViewBackup = new Wt::WPushButton("Details", table->elementAt(row, 5)),
            //click 'PDF' button on any table row, connect downloadReferral with that referral uuid
            btnViewBackup->clicked().connect(boost::bind(&StaffBackupListWidget::showBackupDialog, this, (*i)->buId_));
...
WString StaffBackupListWidget::setCommentId()
{
    return uuid();
}

void StaffBackupListWidget::showBackupDialog(std::string& bu)
{
    ...
    Wt::WPushButton *removeBackup = new Wt::WPushButton("Remove from Backup List");
    contactInfo->addWidget(new Wt::WBreak());
    contactInfo->addWidget(new Wt::WBreak());

    contactInfo->addWidget(removeBackup);


    vbox1->addWidget(contactInfo);
    removeBackup->clicked().connect(std::bind([=] () {
    ...
    vbox2->addWidget(new Wt::WText("<h3>No-Show Backup Patients</h3>"));
