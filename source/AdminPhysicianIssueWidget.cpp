...
AdminPhysicianIssueWidget::AdminPhysicianIssueWidget(const char *conninfo, UserSession *session, WContainerWidget *parent) :
...
void AdminPhysicianIssueWidget::issueList()
{
    ...
    refreshTimer->timeout().connect(std::bind([=] () {
        ...
        AdminPhysicianIssueWidget::search("");
    }));
    ...
    edit->textInput().connect(std::bind([=] () {
       ...
       AdminPhysicianIssueWidget::search(edit->text().toUTF8());

       if (edit->text() == "")
       {
            AdminPhysicianIssueWidget::startRefreshTimer();
       } else {
               AdminPhysicianIssueWidget::stopRefreshTimer();
       }
    ...
    AdminPhysicianIssueWidget::search(Wt::WString(""));
    AdminPhysicianIssueWidget::startRefreshTimer();

}

void AdminPhysicianIssueWidget::preSearch(Wt::WString searchString)
{
    AdminPhysicianIssueWidget::search(Wt::WString(searchString));
    AdminPhysicianIssueWidget::startRefreshTimer();
}
void AdminPhysicianIssueWidget::stopRefreshTimer()
{
...
void AdminPhysicianIssueWidget::startRefreshTimer()
{
...
void AdminPhysicianIssueWidget::search(Wt::WString searchString)
{
    int limit = 100;
    int offset = 0;
    ...
    prContainer->done().connect(std::bind([=] () {
        ...
        AdminPhysicianIssueWidget::preSearch(searchString);
    }));
}
