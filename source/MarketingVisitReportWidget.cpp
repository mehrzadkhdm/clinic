...
MarketingVisitReportWidget::MarketingVisitReportWidget(const char *conninfo, UserSession *session, WContainerWidget *parent) :
...
void MarketingVisitReportWidget::newVisitReport()
{
    ...
    Wt::WText *delabel = new Wt::WText("Please select the date range for the visits report.");
    ...
    button->clicked().connect(std::bind([=] () {
        ...
        MarketingVisitReportWidget::showResult(start, end, pcc);
    }));
...
void MarketingVisitReportWidget::showResult(Wt::WDate& startDate, Wt::WDate& endDate, Wt::WString& pcc)
{

    resultStack_->clear();

    visitReport = new PccVisitReportWidget(conninfo_, session_, resultStack_);
    visitReport->showVisitReport(startDate, endDate, pcc);
    resultStack_->setCurrentWidget(visitReport);
