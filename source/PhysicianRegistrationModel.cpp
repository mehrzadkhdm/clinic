...
const WFormModel::Field PhysicianRegistrationModel::ChoosePasswordField
  = "choose-password";

const WFormModel::Field PhysicianRegistrationModel::RepeatPasswordField
  = "repeat-password";

const WFormModel::Field PhysicianRegistrationModel::EmailField
  = "email";

PhysicianRegistrationModel::PhysicianRegistrationModel(const Wt::Auth::AuthService& baseAuth,
             Wt::Auth::AbstractUserDatabase& users,
             Wt::Auth::Login& login,
             Wt::Auth::User& practice,
             Wt::WObject *parent)
...
void PhysicianRegistrationModel::reset()
{
...
void PhysicianRegistrationModel::setEmailPolicy(EmailPolicy policy)
{
  emailPolicy_ = policy;
  switch (emailPolicy_) {
  case EmailMandatory:
...
bool PhysicianRegistrationModel::registerIdentified(const Identity& identity)
{
  idpIdentity_ = identity;
...
bool PhysicianRegistrationModel::isVisible(Field field) const
{
...
bool PhysicianRegistrationModel::isFederatedLoginVisible() const
{
  return !oAuth().empty() && !idpIdentity_.isValid();
}

bool PhysicianRegistrationModel::isConfirmUserButtonVisible() const
{
  return confirmIsExistingUser() != ConfirmationNotPossible;
}

bool PhysicianRegistrationModel::isReadOnly(Field field) const
{
...
WString PhysicianRegistrationModel::validateLoginName(const WT_USTRING& userName) const
{
  switch (baseAuth()->identityPolicy()) {
  case LoginNameIdentity:
...
void PhysicianRegistrationModel::checkUserExists(const WT_USTRING& userName)
{
  existingUser_ = users().findWithIdentity(Identity::LoginName, userName);
}

bool PhysicianRegistrationModel::validateField(Field field)
{
  if (!isVisible(field))
    return true;
...
PhysicianRegistrationModel::IdentityConfirmationMethod
PhysicianRegistrationModel::confirmIsExistingUser() const
{
...
void PhysicianRegistrationModel::existingUserConfirmed()
{
  if (idpIdentity_.isValid())
...
void PhysicianRegistrationModel::validatePasswordsMatchJS(WLineEdit *password,
             WLineEdit *password2,
             WText *info2)
{
...
User PhysicianRegistrationModel::doRegister()
{
  /*
  practice_.addIdentity(Wt::Auth::Identity::LoginName, valueText(LoginNameField).toUTF8());

  passwordAuth()->updatePassword(practice_, valueText(ChoosePasswordField));
 ...
 if (!passwordAuth() && !idpIdentity_.isValid()) {
    ...
    if (idpIdentity_.isValid()) {
      ...
      user.addIdentity(Identity::LoginName, valueText(LoginNameField));
      user.setEmail(valueText(EmailField).toUTF8());
