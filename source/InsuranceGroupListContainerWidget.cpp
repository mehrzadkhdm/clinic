...
InsuranceGroupListContainerWidget::InsuranceGroupListContainerWidget(const char *conninfo, UserSession *session, WContainerWidget *parent) :
...
void InsuranceGroupListContainerWidget::showSearch(Wt::WString& curstr, int limit, int offset)
{
    ...
    typedef dbo::collection< dbo::ptr<InsuranceGroup> > Insurances;
    Insurances insurances = dbsession.find<InsuranceGroup>().where("group_name LIKE ?").bind(searchPt);
    ...
    table->elementAt(0, 1)->addWidget(new Wt::WText("Insurance Group Name"));
    ...
    for (Insurances::const_iterator i = insurances.begin(); i != insurances.end(); ++i, ++row)
    {
        ...
        new Wt::WText(Wt::WString::fromUTF8("{1}").arg(row),
            ...
            new Wt::WText((*i)->groupName_, table->elementAt(row, 1));
    }
